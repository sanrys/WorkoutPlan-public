# Workout Plan V2 - Hybrid Body-Part Emphasis

**Level:** Advanced | **Equipment:** Full Gym (Equinox) | **Primary Rest Day:** Saturday
**Goals:** Muscle Building + Stronger Legs + Fat Loss + Sustainable Recovery

---

## Why V2

V2 moves away from strict push/pull and into a hybrid body-part emphasis split. The goal is to keep what worked from the push/pull plan while giving legs a clearer growth signal and more recovery.

Key changes:

- Legs are no longer hit hard almost every day.
- Tuesday is the main quad-building day.
- Thursday is the hamstring/glute/posterior-chain day.
- Deadlift/rack-pull fatigue is controlled instead of being a weekly default.
- Cardio and core are built around the lifting instead of competing with it.
- Missed weeks are easier to recover from because Wednesday is a reset/recovery anchor.

---

## Weekly Overview

| Day | Split | Focus |
|-----|-------|-------|
| **Sunday** | Chest + Shoulders + Triceps | Upper push strength and hypertrophy |
| **Monday** | Back + Biceps | Upper pull without heavy leg fatigue |
| **Tuesday** | Legs A | Main quad-building day |
| **Wednesday** | Zone 2 + Core + Mobility | Recovery, reset, visceral-fat support |
| **Thursday** | Legs B | Hamstrings, glutes, posterior chain |
| **Friday** | Upper Pump + Arms + Weak Points | Shoulders, arms, upper volume, optional conditioning |
| **Saturday** | **REST** | Recovery, walking, optional easy Zone 2 |

> **Leg growth rule:** Hard lower-body training happens twice per week: Tuesday for quads and Thursday for hamstrings/glutes. This should create a stronger growth signal than touching legs lightly or heavily on almost every day.

> **Knee considerations (right knee, lateral):** Stop squats around parallel. Do not chase deep squat range if the knee is irritated. Prioritize hack squat, leg press, single-leg leg press, box step-ups, and controlled single-leg RDLs. If discomfort appears, shorten range of motion or use light leg extension in a pain-free range.

> **Missed-day reset rule:** If the week drifts because sessions were skipped or shifted, do not try to make up every missed day. Use the next sensible bridge session based on fatigue and reset back to the normal weekly schedule on Wednesday.

> **V2 rollout rule:** The first clean V2 block starts Sunday, 2026-07-05. If coming directly from the old V1 Thursday Push C session, do not do the normal V2 Friday upper pump the next day. Use a transition day instead: easy Zone 2, rear delts, light rows, arms, core, and mobility. No close-grip bench, Arnold press, skull crushers, hard squats, or deadlifts.

---

## Sunday - Chest + Shoulders + Triceps

| # | Exercise | Sets x Reps | Rest | Suggested Weight |
|---|----------|-------------|------|-----------------|
| 1 | Flat Barbell Bench Press | 4 x 5-6 | 2.5-3 min | 115 lbs |
| 2 | Incline Dumbbell Press | 3 x 8-12 | 90 sec | 35-40 lbs each |
| 3 | Pec Deck / Machine Chest Press | 3 x 10-15 | 75 sec | Controlled squeeze |
| 4 | Seated Dumbbell OHP | 3 x 8-10 | 90 sec | 30 lbs each |
| 5 | Cable Lateral Raise | 3 x 12-15 | 60 sec | 15 lbs |
| 6 | Tricep Rope Pushdown | 3 x 10-12 | 60 sec | 40 lbs |
| 7 | Overhead Cable Tricep Extension | 2-3 x 12-15 | 60 sec | 35-40 lbs |

**Optional finish:** 10-15 min easy Zone 2 if energy is good.

---

## Monday - Back + Biceps

| # | Exercise | Sets x Reps | Rest | Suggested Weight |
|---|----------|-------------|------|-----------------|
| 1 | Pull-Ups / Assisted Pull-Ups | 4 x 6-8 | 2 min | Bodyweight |
| 2 | Lat Pulldown (wide or neutral) | 3 x 8-12 | 90 sec | 100 lbs |
| 3 | Chest-Supported Row / Machine Row | 4 x 8-10 | 90 sec | 100-120 lbs or 40 lbs each |
| 4 | Seated Cable Row | 3 x 10-12 | 75-90 sec | 110-120 lbs |
| 5 | Straight-Arm Pulldown | 3 x 12-15 | 60 sec | 40 lbs |
| 6 | Face Pulls / Reverse Flyes | 3 x 15-20 | 60 sec | 30 lbs or 15 lbs each |
| 7 | EZ-Bar Curl | 3 x 10-12 | 60 sec | 55-60 lbs |
| 8 | Incline DB Curl / Hammer Curl | 2-3 x 10-12 | 60 sec | 20-25 lbs each |

**Intent:** Build the back without adding deadlift fatigue before Tuesday legs.

---

## Tuesday - Legs A: Quad Focus

| # | Exercise | Sets x Reps | Rest | Suggested Weight |
|---|----------|-------------|------|-----------------|
| 1 | Hack Squat | 4 x 8-12 | 2.5 min | 90-115 lbs |
| 2 | Back Squat to Parallel / Safety Bar Squat | 3 x 5-6 | 2.5-3 min | 115-135 lbs |
| 3 | Leg Press | 3 x 10-15 | 90 sec | 180 lbs |
| 4 | Single-Leg Leg Press | 3 x 10-12/leg | 90 sec | 90 lbs |
| 5 | Leg Extension (pain-free range) | 3 x 12-15 | 60 sec | Light-moderate |
| 6 | Standing or Seated Calf Raise | 4 x 10-15 | 60 sec | 135 lbs |

**Rules:** Stop squat depth around parallel. If the knee talks back, reduce squat volume first and keep hack squat/leg press controlled.

**Cardio:** Skip hard intervals. If desired, do 8-12 min very easy bike or incline walk only.

---

## Wednesday - Zone 2 + Core + Mobility

| # | Exercise | Sets x Reps | Rest | Suggested Weight |
|---|----------|-------------|------|-----------------|
| 1 | Zone 2 Cardio | 25-35 min | Easy pace | Row, bike, incline walk, or elliptical |
| 2 | Dead Bug | 2-3 x 8/side | 30 sec | Bodyweight |
| 3 | Pallof Press | 2-3 x 10/side | 45 sec | Light cable |
| 4 | Side Plank | 2 x 20-40 sec/side | 45 sec | Bodyweight |
| 5 | Cable Crunch / Hanging Knee Raise | 2-3 x 10-15 | 60 sec | Controlled |
| 6 | Hip, Hamstring, Upper-Back Mobility | 8-12 min | Easy | Pain-free range |

**Intent:** This is not a hidden hard workout. It is a reset day that supports fat loss, core strength, and recovery before posterior-chain legs.

---

## Thursday - Legs B: Hamstrings + Glutes

| # | Exercise | Sets x Reps | Rest | Suggested Weight |
|---|----------|-------------|------|-----------------|
| 1 | Romanian Deadlift | 3 x 8-10 | 2.5 min | 115 lbs |
| 2 | Barbell Hip Thrust | 4 x 8-12 | 90 sec | 135 lbs |
| 3 | Seated or Lying Leg Curl | 3 x 10-15 | 75 sec | 45 lbs |
| 4 | Single-Leg RDL (dumbbell) | 2-3 x 8-10/leg | 90 sec | 25-30 lbs each |
| 5 | 45-Degree Back Extension (glute-biased) | 3 x 10-12 | 75 sec | Bodyweight or 25 lbs |
| 6 | Cable Hip Abduction / Glute Med Work | 2-3 x 12-15/side | 60 sec | Light-moderate |
| 7 | Calf Raise | 3 x 12-15 | 60 sec | Moderate |

**Optional rotation:** Every other block, replace RDL with trap-bar deadlift or rack pull for 3 x 5-6. Do not run heavy RDLs and heavy rack pulls in the same week.

---

## Friday - Upper Pump + Arms + Weak Points

| # | Exercise | Sets x Reps | Rest | Suggested Weight |
|---|----------|-------------|------|-----------------|
| 1 | Machine Chest Press / Cable Fly | 2-3 x 10-15 | 75 sec | Moderate pump |
| 2 | Landmine Press / Cable Y-Raise | 3 x 10-12 | 90 sec | Light-moderate |
| 3 | Cable Row / Chest-Supported T-Bar Row | 3 x 10-12 | 75 sec | Moderate |
| 4 | Lateral Raises | 3 x 12-20 | 60 sec | 15 lbs each |
| 5 | Rear Delt Flyes | 3 x 15-20 | 60 sec | 15 lbs each |
| 6 | Rope Pushdown + Cable Curl Superset | 3 x 12 each | 60 sec | Controlled |
| 7 | Hammer Curl | 2-3 x 10-12 | 60 sec | 25 lbs each |
| 8 | Cable Crunch / Ab Wheel / Hanging Knee Raise | 2-3 x 10-15 | 60 sec | Controlled |

**Conditioning:** 10-20 min easy Zone 2 after lifting. Grit Runner intervals are optional and only if legs feel fresh.

---

## Saturday - REST DAY

- Walk for active recovery.
- Foam rolling / mobility work, 15-20 min.
- Optional 30-45 min Zone 2 on bike, rower, treadmill, elliptical, or Grit Runner.
- Prioritize sleep, hydration, and protein.

---

## Progressive Overload Strategy

Use **double progression**: hit the top of the rep range with good form across all sets before adding weight. Keep 1-2 reps in reserve on most compound movements. Push closer to failure only on safer isolation work.

| Area | Rep Scheme | Progression Rule |
|------|------------|------------------|
| Heavy compounds | 5-8 reps | Add 2.5-5 lbs when all sets are clean |
| Hypertrophy compounds | 8-12 reps | Add weight after owning the top of the range |
| Isolation work | 12-20 reps | Add reps first, then small weight increases |
| Legs | 8-15 reps mostly | Prioritize controlled reps, ROM tolerance, and recovery |

**Deload schedule:** V2 training block starts **2026-07-05**. Weeks 1-4 are normal training. Week 5 is a deload check week: deload if performance is down for 2+ sessions, joints feel cranky, sleep/recovery is poor, motivation is unusually low, or warm-up weights feel heavier than normal.

**Deload method:** Deload does not mean starting over. Reduce sets by about 30-40%, keep technique crisp, and use slightly easier loads only if joints or bar speed ask for it. If you feel good in Week 5, keep normal weights but stop 2-3 reps shy of failure and avoid extra volume.

---

## Conditioning for Fat Loss + Visceral Fat

Conditioning should support the lifting, not crush leg growth.

| Type | Frequency | Prescription |
|------|-----------|--------------|
| **Zone 2 after strength** | 2-3x/week | 10-20 min easy pace after lifting |
| **Longer Zone 2** | 1-2x/week | 25-45 min on Wednesday/Saturday |
| **Grit Runner Intervals** | 0-1x/week | 6-8 rounds: 20 sec hard + 70-100 sec easy |
| **Steps** | Daily | Aim for 8,000-10,000+ steps |

Rowing is fine for Zone 2, but keep it easy. Bike, incline walk, and elliptical are often easier to recover from after leg sessions.

---

## Core / Abs Placement

- Before lifting: 5-8 min activation only, such as dead bugs, Pallof press, plank, or side plank.
- Harder ab work: Wednesday and/or Friday after lifting/cardio.
- Avoid fatiguing abs before squats, RDLs, rows, or overhead pressing.

Core work will strengthen and shape the midsection, but belly fat reduction still depends mostly on calorie balance, steps, Zone 2, sleep, and consistency.

---

## Version Switching

The old push/pull plan is stored as `plan-versions/v1/`.
This hybrid plan is stored as `plan-versions/v2/`.

To switch the live files later:

```bash
scripts/switch-plan.sh v1
scripts/switch-plan.sh v2
```

After switching, review the files, commit, push `github main`, and publish the public files through the GitHub API.

---

## Nutrition Guidelines

| Factor | Target |
|--------|--------|
| **Calories** | Slight deficit, about 200-300 below maintenance, for lean recomp |
| **Protein** | 1g per lb bodyweight minimum |
| **Carbs** | Keep enough carbs around leg days to train hard |
| **Fats** | 0.3-0.4g per lb bodyweight |
| **Water** | 1 gallon/day target |
| **Sleep** | 7-9 hours for recovery |

---

*Disclaimer: Training and nutrition suggestions are general guidelines. Adjust around pain, recovery, and medical guidance where relevant.*
